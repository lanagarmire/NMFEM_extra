...
leading_genes[['Monocle']] <- readRDS('leading_genes/leadingGenesMonocle_B2013.RDS')
...
leading_genes[['MAST']] <- readRDS('leading_genes/leadingGenesMAST_B2013.RDS')
...
for (method_index in 1:length(leading_genes)) {
  ...
  write.table(final_tb, paste0('out-modules/', names(leading_genes)[method_index], '.txt'), sep='\t', row.names=F, col.names=T, quote=F)
}
